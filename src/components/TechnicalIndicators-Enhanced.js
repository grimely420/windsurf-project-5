...
const TechnicalIndicators = ({ priceHistory, currentPrice }) => {
  ...
  // Get price history from data with enhanced validation
  ...
  const calculateMovingAverages = (prices) => {
    ...
    return {
      ...
      ema50: calculateEMA(prices, 50)[0]
    };
  };
  ...
  const getSignal = () => {
    ...
    if (rsi > 70) signals.push({ type: 'sell', source: 'RSI', strength: 'strong', text: 'Overbought' });

    // MACD signals
    if (macd.histogram > 0) signals.push({ type: 'buy', source: 'MACD', strength: 'medium', text: 'Bullish crossover' });
    if (macd.histogram < 0) signals.push({ type: 'sell', source: 'MACD', strength: 'medium', text: 'Bearish crossover' });
      ...
      <div className="indicators-grid">
        ...
        <div className="indicator-card">
          ...
          <div className="indicator-visual">
            <div className="macd-line">
              <div className="macd-histogram" style={{ 
                height: `${Math.abs(indicators.macd.histogram) * 20}px`,
                backgroundColor: indicators.macd.histogram > 0 ? '#10b981' : '#ef4444'
              }}></div>
            </div>
          </div>
        </div>
        ...
        <div className="indicator-card">
          ...
          <div className="bb-visual">
            <div className="bb-band upper" style={{ 
              bottom: `${((indicators.bollingerBands.upper - indicators.bollingerBands.middle) / indicators.bollingerBands.middle) * 100}%` 
            }}></div>
            <div className="bb-band middle" style={{ 
              bottom: `${((indicators.bollingerBands.middle - safeCurrentPrice) / indicators.bollingerBands.middle) * 100}%` 
            }}></div>
            <div className="bb-band lower" style={{ 
              top: `${((safeCurrentPrice - indicators.bollingerBands.lower) / indicators.bollingerBands.middle) * 100}%` 
            }}></div>
          </div>
        </div>
      </div>
